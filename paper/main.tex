...
\subsubsection{Datasets and Two-Stage Training Strategy}

We train two parallel DDPM instances targeting different visual aspects of each creature, then apply a shared fine-tuning stage to inject fantasy aesthetics:

\textbf{DDPM-Face} (portrait generation):
\begin{itemize}
    \item \textbf{Stage 1}: Pre-trained on AFHQ v2~\cite{choi2020starganv2} --- 15{,}000 high-quality animal face photographs across three categories (cat, dog, wildlife) at $512 \times 512$ resolution, resized to $64 \times 64$. This model learns fine-grained facial morphology: fur texture, eye structure, ear shape, and facial proportions.
    \item \textbf{Stage 2}: Fine-tuned on Dungeons \& Diffusion~\cite{dungeons_diffusion} --- approximately 2{,}500 fantasy character art images across 30 races. Learning rate is reduced to $5 \times 10^{-5}$ (from $2 \times 10^{-4}$) to preserve learned features while shifting toward fantasy illustration style.
\end{itemize}

\textbf{DDPM-Body} (full-body illustration):
\begin{itemize}
    \item \textbf{Stage 1}: Pre-trained on AWA2~\cite{xian2018zero} --- 37{,}000 full-body animal photographs across 50 classes (lion, elephant, butterfly, etc.). This model learns complete body morphology: limb structure, body proportions, pose, and silhouette variety.
    \item \textbf{Stage 2}: Fine-tuned on Dungeons \& Diffusion under the same transfer protocol as DDPM-Face.
\end{itemize}

Both models share the identical U-Net architecture and diffusion schedule; only the training data differs. During fine-tuning, compatible convolutional weights are transferred from the Stage~1 checkpoint, while class-conditional layers are reinitialised to accommodate the new label space. The dual-model design enables each creature in the encyclopedia to be presented with both a \emph{portrait} and a \emph{full-body illustration}, mirroring the visual conventions of real zoological field guides. We compare Stage~1-only and Stage~1+2 outputs for both models in Section~\ref{sec:evaluation}.
...
The training corpus is designed to teach the model the register and structure of formal taxonomic encyclopedias, following the conventions of Wikipedia zoological articles. It is constructed from two complementary sources:

\begin{itemize}
    \item \textbf{Wikipedia animal articles} ($\sim$50 species): Introductory extracts for real species spanning 10 taxonomic classes (Mammalia, Aves, Reptilia, Amphibia, Chondrichthyes, Actinopterygii, Cephalopoda, Insecta, Arachnida, Scyphozoa), fetched via the Wikipedia REST API. These provide authentic zoological prose covering morphology, behaviour, ecology, and conservation.
    \item \textbf{Fictional creature descriptions} (10 species): Handcrafted multi-paragraph entries written in the same formal taxonomic register, describing invented organisms with internally consistent biology (e.g.\ antifreeze glycoproteins in a glacial annelid, iron-oxide-infused keratin in a volcanic reptile). These teach the model to generate novel yet plausible species descriptions.
\end{itemize}

Each entry follows a structured template with full Linnaean classification metadata:

\begin{verbatim}
<SPECIES>
Common Name: Ironscale Basilisk
Scientific Name: Ferrovaranus ignicola
Kingdom: Animalia
Phylum: Chordata
Class: Reptilia
Order: Squamata
...
---
The Ironscale Basilisk (Ferrovaranus ignicola) is a large
thermophilic reptile endemic to active volcanic ridges...
</SPECIES>
\end{verbatim}

This structured format serves a dual purpose: the metadata fields act as conditioning context during prompt-based generation, while the delimiter tokens (\texttt{<SPECIES>}, \texttt{---}, \texttt{</SPECIES>}) enable reliable parsing of model outputs. The corpus is repeated 5$\times$ to increase training density for the character-level model (total $\sim$600K characters).
