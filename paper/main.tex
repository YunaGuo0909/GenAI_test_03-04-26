...
Rather than training on a single dataset, we adopt a \textbf{two-stage transfer learning} approach to bridge the domain gap between photorealistic animal imagery and fantasy creature aesthetics:

\begin{enumerate}
    \item \textbf{Stage 1 --- Pre-training on AFHQ v2} (real-world domain). The primary dataset is AFHQ v2~\cite{choi2020starganv2}, comprising approximately 15{,}000 high-quality animal photographs across three categories (cat, dog, wildlife) at $512 \times 512$ resolution. Images are resized to $64 \times 64$ pixels. This stage teaches the model the fundamental visual distribution of real animal morphology --- fur texture, eye structure, facial proportions --- with a large and clean dataset.

    \item \textbf{Stage 2 --- Fine-tuning on Dungeons \& Diffusion} (fantasy domain). We then fine-tune the pre-trained model on the Dungeons \& Diffusion dataset\footnote{\url{https://huggingface.co/datasets/0xJustin/Dungeons-and-Diffusion}}, a collection of approximately 2{,}500 fantasy character art images across 30 races (e.g.\ dragonborn, tiefling, kobold). The fine-tuning uses a reduced learning rate ($5 \times 10^{-5}$, down from $2 \times 10^{-4}$) to preserve learned features while shifting the visual style toward fantasy illustration. Compatible U-Net weights are transferred; class-conditional layers are reinitialised to accommodate the new label space.
\end{enumerate}

This strategy is motivated by the observation that the fantasy art dataset alone is too small to train a DDPM from scratch at acceptable quality, while the AFHQ-only model produces photorealistic outputs that lack the desired fantastical character. The two-stage approach allows us to leverage the rich visual priors learned from real animals while adapting the generative distribution toward a stylised fantasy domain. We compare Stage~1-only and Stage~1+2 outputs in Section~\ref{sec:evaluation}.
